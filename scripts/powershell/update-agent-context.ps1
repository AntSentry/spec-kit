...
switch ($AgentType) {
    ...
    'codex' { Update-AgentFile $codexFile 'OpenAI Codex' }
    '' {
        foreach ($pair in @(
            ...
            @{file=$codexFile; name='OpenAI Codex'}
        )) {
            if (Test-Path $pair.file) { Update-AgentFile $pair.file $pair.name }
        }
        if (-not (Test-Path $claudeFile) -and -not (Test-Path $geminiFile) -and -not (Test-Path $copilotFile) -and -not (Test-Path $cursorFile) -and -not (Test-Path $qwenFile) -and -not (Test-Path $agentsFile) -and -not (Test-Path $codexFile)) {
    ...
    Default { Write-Error "ERROR: Unknown agent type '$AgentType'. Use: claude, gemini, copilot, cursor, qwen, opencode, codex or leave empty for all."; exit 1 }
}
...
Write-Output ''
Write-Output 'Usage: ./update-agent-context.ps1 [claude|gemini|copilot|cursor|qwen|opencode|codex]'
