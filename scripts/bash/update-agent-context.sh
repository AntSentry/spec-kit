...
CLAUDE_FILE="$REPO_ROOT/CLAUDE.md"; GEMINI_FILE="$REPO_ROOT/GEMINI.md"; COPILOT_FILE="$REPO_ROOT/.github/copilot-instructions.md"; CURSOR_FILE="$REPO_ROOT/.cursor/rules/specify-rules.mdc"; QWEN_FILE="$REPO_ROOT/QWEN.md"; AGENTS_FILE="$REPO_ROOT/AGENTS.md"; CODEX_FILE="$REPO_ROOT/CODEX.md"
AGENT_TYPE="$1"
...
case "$AGENT_TYPE" in
  ...
  codex) update_agent_file "$CODEX_FILE" "OpenAI Codex" ;;
  "") [ -f "$CLAUDE_FILE" ] && update_agent_file "$CLAUDE_FILE" "Claude Code"; \
       ...
       [ -f "$CODEX_FILE" ] && update_agent_file "$CODEX_FILE" "OpenAI Codex"; \
       if [ ! -f "$CLAUDE_FILE" ] && [ ! -f "$GEMINI_FILE" ] && [ ! -f "$COPILOT_FILE" ] && [ ! -f "$CURSOR_FILE" ] && [ ! -f "$QWEN_FILE" ] && [ ! -f "$AGENTS_FILE" ] && [ ! -f "$CODEX_FILE" ]; then update_agent_file "$CLAUDE_FILE" "Claude Code"; fi ;;
